...
> Обновлено: 2026-08-12. Статус: **план согласован, работа не начата.**
> Следующий шаг — разбить фазы на GitHub issues.
...
| Файл | Строки |
|---|---|
| `app/Http/Controllers/Api/V1/Admin/ReportSpeakerController.php` | 18, 37, 54, 62 |
| `app/Http/Controllers/Api/V1/Admin/ReportModeratorController.php` | 18, 38, 50 |
| `app/Http/Controllers/Api/V1/Admin/ExportReportUserController.php` | 22, 46 |
| `app/Console/Commands/ExportReportCommand.php` | 17, 133, 143, 191 |
| `app/Console/Commands/ExportDailyReportCommand.php` | 18, 131, 150 |

Если не исправить — после переключения все отчёты по спикерам и модераторам продолжат
...
Отдельный PR, деплоится **до** загрузки датасета.

- [ ] Заменить 5 констант `FILE_ID = 8` на `config('dataset.main_file_id')` (список в §2Б)
...
## 5. Разбивка на issues (черновик)

| Issue | Фаза | Содержание |
|---|---|---|
| 1 | 0 | Закрыть экспорт и модерацию `file_id = 8` |
| 2 | 1 | `FILE_ID = 8` → конфиг в 5 файлах отчётов |
| 3 | 1 | Фильтр активного датасета в `DailyQuotaController`, `ProfileReportController`, `Verify/UserController` |
| 4 | 1 | Фильтр файла в удерживаемом тексте `SpeakTextController` + очередях модерации |
| 5 | 1 | Конфигурируемый `--file-id` в командах экспорта |
| 6 | 2 | Импорт нового датасета (+ чанковый `insert`, проверка дублей) |
| 7 | 3 | Переключение прода и постпроверка |
| 8 | 4 | `files.label` / `is_active` и отображение в админке |
