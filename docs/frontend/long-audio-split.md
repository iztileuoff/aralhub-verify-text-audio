...
## 5. Исправление текста разрезанной части

```
PATCH /api/v1/admin/long/audio/part/{text}/text
Content-Type: application/json
```

Иногда **аудио** разрезано верно, а **текст** поделён на части неправильно. Этот endpoint
правит транскрипт одной части прямо на экране проверки результатов — **аудио не трогается**,
оригинальный (родительский) текст тоже остаётся как есть (эталон для сверки).

`{text}` — `parts[].text_id` из ответа шага 4 (id дочернего текста части).

**Body:**

| Поле | Тип | Обяз. | Правила |
|------|-----|-------|---------|
| `text` | string | да | новый текст части; бэкенд сам выводит `normalized` и `tokenized` |

**Успех `200`:** ресурс текста с обновлёнными `edit_original_transcript`,
`edit_normalized_transcript`, `edit_tokenized_transcript`.

**Ошибки:**

| Код | Когда | Тело |
|-----|-------|------|
| `422` | нет поля `text` | `{ "message": "...", "errors": { "text": ["..."] } }` |
| `404` | текст не является частью разрезанного (`SPLIT`) аудио основного датасета | — |

**Пример (curl):**

```bash
curl -X PATCH ".../api/v1/admin/long/audio/part/902/text" \
  -H "Authorization: Bearer $TOKEN" -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  -d '{"text":"исправленная первая половина текста"}'
```

> Доступно только для дочерних текстов частей (`is_split_part`) разрезанных аудио основного
> датасета. Поправить можно обе части — по очереди, каждую своим `text_id`.
...
8. Если текст части поделён неверно (а аудио — верно), исправить транскрипт части
   через `PATCH long/audio/part/{text}/text` (шаг 5), не перерезая аудио.
